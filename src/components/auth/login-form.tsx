...
export function LoginForm({
    ...
    const errorMessage = getErrorMessage(errorId)
            ...
            <div className="space-y-6">
                ...
                <form
                    action={async (formData) => {
                        "use server"
                        const errorPath = formData.get("errorRedirectPath")?.toString() || "/login"
                        const successPath = formData.get("redirectTo")?.toString() || "/learner-dashboard"
                        try {
                            ...
                            if (error instanceof AuthError) {
                                return redirect(`${errorPath}?error=${error.type}`)
                            }
                            throw error
                        }
                    }}
